...
if ( $uploaded_files ) {
	foreach ( $uploaded_files as $index => $value ) {
		...
		if ( ! empty( $value['image'] ) ) {
			$files[ $index ] = $value['image'];
		}
		
		unset( $uploaded_files[ $index ]['image'] );
	}
}
					...
					<div class="field ">	
						...
						<?php if ( is_admin() ) : ?>
							<div class="file-upload-field single-upload form-group-review-gallery">
								<div class="uploaded-files-list review-gallery-images">
									<div class="upload-file review-gallery-add listing-file-upload-input" data-name="image" data-multiple="">
										<i class="mi file_upload"></i>
										<div class="content"></div>
									</div>
									<input type="hidden" class="input-text outer-photo" name="image">
						...
						<?php else : ?>
							<div class="file-upload-field single-upload form-group-review-gallery ajax-upload">
								...
								name="image"
								id="<?php echo esc_attr( (isset($field['name']) ? $field['name'] : $key) ) ?>_image"
								style="display: none;"
								>
								<div class="uploaded-files-list review-gallery-images">
									<label class="upload-file review-gallery-add" for="image">
										<i class="mi file_upload"></i>
										<div class="content"></div>
									</label>
